...
/* 0xff */
make_group(gp5,
    EX(inc), EX(dec), IDEX(J_indirect, call), EMPTY,
    IDEX(J_indirect, jmp), EMPTY, EX(push), EMPTY)
...
static OpcodeEntry opcode_table [512] = {
  ...
  /* 0x58 */	IDEX(s2r, pop), IDEX(s2r, pop), IDEX(s2r, pop), IDEX(s2r, pop),
  /* 0x5c */	IDEX(s2r, pop), IDEX(s2r, pop), IDEX(s2r, pop), IDEX(s2r, pop),
  ...
  /* 0x8c */	EMPTY, IDEX(lea_E2G, lea), EMPTY, EMPTY,
  ...
  /* 0xec */	EMPTY, IDEX(in_dx2a, in), IDEXW(out_a2dx, out, 1), IDEX(out_a2dx, out),
